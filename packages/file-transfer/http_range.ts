...
export async function getWithRange({
  url,
  headers,
  range,
  dispatcher,
  signal,
  blackhole,
}: GetWithRangeOptions): Promise<HTTPResourceOrStatusError> {
  ...
  await stream<Opaque>(
    ...
    ({ statusCode, headers, context, opaque }) => {
      ...
      const redirectedUrl =
        'history' in context && context.history instanceof Array
