...
function generate(packagePath) {
    ...
    const result = [{
        ...
        external: (source, importer, isResolved) => {
            if (external.indexOf(source) !== -1) { return true }
    ...
    if (packageJson.browser) {
        result.push({
            input: existedBrowserEntry ? `${packagePath}/index.browser.ts` : `${packagePath}/index.ts`,
            output: [{
...
const options = workspaces.map(generate).reduce((all, cur) => all.concat(cur));
