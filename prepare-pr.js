...
async function main() {
    ...
    const newVersion = packages.find(pack => pack.package.name === '@xmcl/minecraft-launcher-core')
        .newVersion
    fs.writeFileSync('pr_title.log', prTitle(newVersion));
    fs.writeFileSync('pr_body.log', prBody(packages));
    fs.writeFileSync('commit_message.log', commitMessage(newVersion));
}
