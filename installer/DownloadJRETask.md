...
```ts
DownloadJRETask(jre: DownloadInfo, dir: string, options: InstallJavaOptions): DownloadJRETask
```
#### Parameters
...
```ts
map(transform: Transform<DownloadJRETask, N>): Task<N extends Promise<R> ? R : N>
```
#### Parameters
