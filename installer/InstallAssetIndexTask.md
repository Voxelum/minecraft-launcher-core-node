...
```ts
InstallAssetIndexTask(version: ResolvedVersion & { assetIndex: AssetIndex }, options: AssetsOptions= {}): InstallAssetIndexTask
```
#### Parameters

- **version**: `ResolvedVersion & { assetIndex: AssetIndex }`
...
<strong>Defined in:</strong> <a href="https://github.com/voxelum/minecraft-launcher-core-node/blob/master/packages/installer/minecraft.ts#L481" target="_blank" rel="noreferrer">packages/installer/minecraft.ts:481</a>
</p>
...
```ts
map(transform: Transform<InstallAssetIndexTask, N>): Task<N extends Promise<R> ? R : N>
```
#### Parameters
